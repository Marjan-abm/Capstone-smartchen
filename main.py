...
def menu_update_insert_by_json_file(operation):
    ...
    while True:
        if operation == UPDATE:
            choice = input('\nChoose one of the following options:\n'
                        ...
                        #    '2 = Update favorite recipes table\n'
                           'b = GO BACK\n\n')
        if operation == INSERT:
            choice = input('\nChoose one of the following options:\n'
                           '1 = Insert all recipes table\n'
                        #    '2 = Insert favorite recipes table\n'
                           'b = GO BACK\n\n')
        if choice == OPTION_BACK:
            show_menu()
            return
    ...
    if option == OPTION_TWO:
        ...
        if operation == UPDATE:
            # 2 = Update favorite recipes table
            update_by_json_file(MONGO_DB, json_file, FAVORITES)
        if operation == INSERT:
            # 2 = Insert favourite recipes table
            insert_by_json_file(MONGO_DB, json_file, FAVORITES)
    show_menu()
