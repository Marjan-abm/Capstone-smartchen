...
def export_to_json_file(mongo_db):
    ...
    if not mongo_db:
        ...
        return
    dictionary = {'all recipes': [], 'favorites': []}
    # get recipes in all_recipes_tb
    for recipe_info in mongo_db.all_recipes.find({}, {'_id': 0}):
        recipe_info_dict = json.loads(dumps(recipe_info))
        dictionary['all recipes'].append(recipe_info_dict)
    # get recipes in favorites_tb
    for recipe_info in mongo_db.favorites.find({}, {'_id': 0}):
        recipe_info_dict = json.loads(dumps(recipe_info))
        dictionary['favorites'].append(recipe_info_dict)
    # write to json file
...
def update_by_json_file(mongo_db, json_file, table_type):
    ...
    table_type (int): flag indicating the type of table in database, all_recipes or favorites tables
    """
    if not mongo_db:
        print('Error: mongo_db given is None')
        return
